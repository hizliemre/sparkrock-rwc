...
`TestEntity` predates these conventions and violates several. It **stays in the codebase** — F13, which would have removed it, is cancelled — but it is **not the reference slice**: prefer `src/features/Schools/` (CRUD) and `src/domain/ValueObjects/SchoolYear.cs` (domain) as the current examples. Its value is that its tests cover the interceptor, the reflective soft-delete filter and the InMemory factory without depending on any business feature.
