...
**`Evidence-kind`** names the *mechanism* that proves the row, from a closed set — `test`, `analyzer`, `migration-inspection`, `report`, `none` — optionally qualified with the tier (`test (integration)`) where only that tier can carry the assertion. `Verified by` then names the *instance*: for `test`, a fully-qualified test name once implemented and `*pending*` until then; for every other kind, the artifact itself. `—` is legitimate only on kind `none`, whose `New behaviour` cell must say why nothing can verify it. The cross-reference check ([features/README](../features/README.md)) applies its test-name rule to `test` rows only — before the column existed, a prose description sat where a mechanism belonged and the check would have failed on every one of them (O-33). That check is **not** mechanised; it is run by hand, and README now says so rather than carrying a ⚙.

**Status after the shipment.** Every `test` row names a test verified to exist, and each was opened and read rather than taken from a spec's claim — three consecutive features had previously recorded an `ErrorCodes` area file as shipped when it did not exist, and one row here cited a test (`AttendanceCodeConstraintTests…`) that has never been written. Twenty-two rows moved `proposed` → `implemented`. The four **verified** rows are pure-domain ones that need nothing from cutover; the rest wait for cutover §5, which is the step that promotes `implemented` to `verified`. Two rows stay `proposed` because their evidence is not a test and has not been produced: V-10's catalogue inspection and V-18's reconciliation report, the latter requiring F12.

| # | Fixes | Legacy behaviour | New behaviour | Feature | Evidence-kind | Verified by | Reversible | Status | Signed off |
|---|---|---|---|---|---|---|---|---|---|
| V-01 | L-01 | Stale `@ExistingID` overwrites another student's row | Each student resolved independently | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenOneStudentHasAnExistingRowAndAnotherDoesNot_UpdatesOnlyTheFirstAndInsertsTheSecond` | n/a | implemented | — |
| V-02 | L-02 | Stale absence flags leak between students | Flags resolved per record | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenEntriesHaveDifferentCodes_SnapshotsEachEntrysOwnFlags` | n/a | implemented | — |
| V-03 | L-03, L-13 | Procedure uncreatable; no transaction | Entire submission in one transaction | F07 | test (integration) | `SaveDailyAttendanceIntegrationTests.SaveChangesAsync_WhenAttendanceInsertViolatesUniqueIndex_RollsBackSummaryAlertAndLog` | n/a | implemented | — |
| V-04 ● | L-06 | Unrecognised code stored as present | Submission rejected | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenAttendCodeIsUnknown_ThrowsBusinessRuleExceptionWithUnknownCode` | config toggle | implemented | — |
| V-05 | L-04 | XML payload interpolated into SQL | JSON request model; banned-API analyzer forbids raw SQL in `features` | F07, F01a2 | analyzer | `src/features/BannedSymbols.txt` — the `T:…RelationalQueryableExtensions` and `T:…RelationalDatabaseFacadeExtensions` **type** bans. Not `FromSql*`/`ExecuteSql*` member entries, which this cell named until F07 shipped: the extension methods live on those two types and banning the type is what reaches all of them. Behavioural half: `SaveDailyAttendanceHandlerTests.Handle_WhenNotesContainSqlMetacharacters_StoresThemVerbatimAndEchoesNothing` | no | implemented | — |
| V-06 ● | L-05 | Dedup key `(StudentID, AttendDate)`; recount filtered by school | Key deliberately unchanged — one record per student per day. The school disagreement is resolved by validating membership, not by widening the key | F07 | test (integration) | `SaveDailyAttendanceIntegrationTests.SaveChanges_WhenTwoSchoolsSubmitTheSameStudentAndDate_ViolatesTheStudentDateUniqueIndex` | no | implemented | — |
| V-07a | L-08 | Per-student recount in cursor | One grouped recount for submitted students — a performance change with no behavioural difference | F07 | test (integration) | `SaveDailyAttendanceIntegrationTests.Handle_IssuesExactlyOneRecountQueryForTheWholeBatch` | n/a | implemented | n/a |
| V-07b | L-12 | Recount predicate filters nothing | Recount scoped to the school year by date range | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenStudentHasAbsencesInThePriorSchoolYear_ExcludesThemFromTheTotal` | no | implemented | — |
| V-07c ● | D-05 | Recount filtered to one school | Recount spans schools within the year | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenStudentHasAbsencesAtAnotherSchoolThisYear_IncludesThemInTheTotal` | no | implemented | — |
| V-08 ● | L-07 | Alerts never resolve | Manual resolve + auto-resolve strictly below threshold; a manual resolution is never auto-re-raised (DEC-18) | F01b, F07, F10 | test | `AlertRulesRaiseTests.ShouldRaise_WhenManuallyResolvedThisYear_ReturnsFalse` (F01b, the rule — class in `Domain/AlertRulesTests.cs`); `SaveDailyAttendanceAlertTests.Handle_WhenCorrectionDropsTotalBelowThreshold_AutoResolvesTheOpenEpisode` and `…Handle_WhenManuallyResolvedThisYearAtThisSchool_DoesNotRaise` (F07, the save path applying it); `ResolveAlertPersistenceTests.Resolve_WhenEpisodeResolved_AllowsANewEpisodeForTheSameKey` (F10, integration — the episode index permits the *next* episode, which is what makes resolution non-terminal) | yes | implemented | — |
...
| V-10 | L-11 | No foreign keys, no non-key indexes | Foreign keys plus indexes on every lookup path | F01c, F01d | migration-inspection | applied DDL of migrations 1 and 2 — **not yet recorded.** What ships instead is partial and is at the wrong level: `MigrationDdlTests.Migration_MakesEverySchoolForeignKeyRestrict` covers the FK *delete behaviour* over the migration text, and index existence is asserted over model metadata (`ReferenceModelTests.Model_EveryLegacyEntityDeclaresItsLegacyIndex`, `…Model_SchoolTermHasTheDateRangeIndexSupportingOverlapChecks`, `ConstraintRegistryTests.Registry_KeyNamesAnIndexTheSchemaCreates`), never against `pg_indexes`/`pg_constraint`. The catalogue inspection F01c's tasks describe remains outstanding | no | proposed | n/a |
...
| V-12 | L-09 | Non-sargable `CASE` in `WHERE` | Half-open date-range predicate | F08 | test (integration) | `StudentAttendanceHistoryPlanTests.Explain_WhenFilteredByDateRange_UsesIndexOnStudentIdAndAttendDate` — a real `EXPLAIN` over the handler's own query, asserting the index by name and the absence of `Seq Scan on student_attendances`. `…_PredicateIsARangeOnAttendDate` additionally bans `CASE`, `extract(` and `date_part` from the emitted `WHERE` | n/a | implemented | n/a |
| V-13 ● | — | The *procedure* accepted back-dated entry for a former school; the *form* could not back-date at all (L-16) | Rejected — membership is current-school only | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenStudentHasTransferredAway_ReportsNotOnRoster` | no | implemented | — |
| V-14 ● | — | Inactive codes and inactive schools accepted | Rejected | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenSchoolIsInactive_ThrowsConflict` (school half, 409); `SaveDailyAttendanceHandlerTests.Handle_WhenAttendCodeIsInactive_ProducesTheSameViolationAsUnknown` (code half, 400 — conventions §2 supersedes this row's original 409 for codes) | config toggle | implemented | — |
| V-15 | — | Duplicate student in one payload upserts twice | Rejected | F07 | test | `SaveDailyAttendanceValidatorTests.Validate_WhenStudentIdAppearsTwice_Fails` | no | implemented | n/a |
...
| V-17 ● | D-05 | Threshold via `summary.SchoolID → Schools` | Threshold resolved from the student's **current** school (`Student.SchoolId`), per DEC-16. The summary's `SchoolId` is retained as school of record, for filtering only — it no longer governs the threshold, so after a transfer the receiving school's threshold decides whether the student is chronic | F09 | test | `GetStudentAbsenteeismHandlerTests.Handle_ResolvesThresholdFromTheStudentsCurrentSchool` — former/summary school threshold 20, current school 5, ten absences; legacy's sourcing would return 20 and *not chronic*. Companion `…Handle_ThresholdSourceIsCurrentSchool` pins the `thresholdSource` discriminator DEC-16 returns instead of the school id | no | implemented | — |
...
| V-19 | D-03 | Overlapping terms resolve arbitrarily | Overlaps rejected at write | F04 | test | `CreateSchoolTermHandlerTests.Handle_WhenDatesOverlapAnActiveTerm_ThrowsConflict`; `UpdateSchoolTermHandlerTests.Handle_WhenReactivatingIntoAnOverlap_ThrowsConflict` — reactivation is the half that is easy to leave unguarded. Boundary matrix in `TermOverlapTests` | no | implemented | n/a |
| V-20 | D-08 | Whole grid always submitted | Partial upsert; omitted students untouched | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenAStudentIsOmittedFromThePayload_LeavesTheirAttendanceAndSummaryUntouched` — asserts `ModifiedAt` is still null on both the attendance row and the summary, so "untouched" means not rewritten with identical values | no | implemented | — |
...
| V-22 | — | `ResolvedBy VARCHAR(100)` | `Guid?`, and **no `LegacyResolvedBy`** — O-34 is resolved in favour of V-18: alerts are recomputed, never imported, so there is no legacy username to preserve | F10 | test (integration) | `ResolveAlertPersistenceTests.Schema_StudentAlertsResolvedByIsUuidAndHasNoLegacyResolvedByColumn` — reads `information_schema.columns` through `DatabaseProbe` and asserts both halves: `resolved_by:uuid` present, no column named `legacy_resolved_by` | no | implemented | n/a |
| V-23 ● | D-02 | Code description joined at read time | Description snapshotted onto the attendance row; redefining a description no longer changes historical display | F01d | test | `StudentAttendanceSnapshotTests.Snapshot_WhenAttendanceCodeIsRedefined_StoredRowIsUnchanged` (write-once under redefinition); `StudentAttendanceSnapshotTests.Model_SnapshotColumnsAreStoredNotComputed` (all four snapshot columns are `ValueGenerated.Never` with no computed SQL) | no | implemented | — |
| V-24 | L-15 | Grade filter silently inert; empty grade always passed | `?grade=` is an explicit optional filter | F06 | test | `GetAttendanceRosterHandlerTests.Handle_WhenGradeFilterIsEmpty_ReturnsAllGrades` — a theory over `""` and whitespace, the exact input legacy always sent; `…Handle_WhenGradeFilterSupplied_ReturnsOnlyThatGrade` is the half legacy never reached | no | implemented | n/a |
| V-25 ● | L-16 | Date fixed at form load; no back-dating | Date is an explicit request parameter, bounded (not future, within a configured back-dating window) | F07 | test | `SaveDailyAttendanceHandlerTests.Handle_WhenDateIsAfterSchoolLocalToday_ThrowsBusinessRuleException` — school-local, so it also fails if anyone reaches for `UtcNow.Date`; `…Handle_WhenDateIsOlderThanTheBackDatingWindow_ThrowsBusinessRuleException` and `…Handle_WhenDateIsWithinTheBackDatingWindow_Succeeds` bound the other end | config toggle | implemented | — |
...
| V-27 ● | — | SQL Server's case-insensitive collation treated `A` and `a` as the same attendance code | Postgres unique indexes are case-sensitive, so both could coexist. A check constraint forces uppercase, making the codes behave as before | F01c, F03 | test | `MigrationDdlTests.Migration_DeclaresCheckConstraintWithItsPinnedNameAndPredicate` (F01c — the DDL declares `ck_attendance_codes_value_upper` as `value = upper(value)`); `CreateAttendanceCodeHandlerTests.Handle_NormalisesValueToUpperCase` and `AttendanceCodeValueTests.Normalise_UsesInvariantCulture` (F03 — the write boundary, and the `tr-TR` dotless-i trap); `SeedWriterPostgresTests.ALowercaseAttendanceCodeValue_IsRejectedByTheCheckConstraint` (integration — the constraint actually firing on the deployed schema). **The test F03's tasks name for the database half, `AttendanceCodeConstraintTests.Insert_WhenValueDiffersOnlyByCase_ConflictsWithTheExistingCode`, does not exist**; the seed test above is the only place the constraint is exercised against Postgres | no | **implemented** | — |
| V-28 ● | — | No access control of any kind: `sp_GetStudentAttendance` filtered on `@StudentID` and the year with no school predicate, so any caller could read any student's history and a former school kept everything it had recorded | Read access follows `Student.SchoolId` (DEC-16). At transfer the former school loses the student's history, absenteeism figure and alerts — including an alert it raised and was mid-way through triaging. The obvious repair, "authorised for the current school **or** any row's school", is rejected: it is an existence-and-prior-relationship oracle on the endpoint DEC-15 exists to harden | F08, F09, F10 | test | `GetStudentAttendanceHistoryHandlerTests.Handle_WhenStudentTransferred_AuthorisesAgainstCurrentSchool` (F08); `GetStudentAbsenteeismHandlerTests.Handle_WhenStudentTransferred_AuthorisesAgainstCurrentSchool` (F09); `ResolveAlertHandlerTests.Handle_WhenCallerHoldsOnlyTheAlertsSchool_ThrowsNotFound` (F10 — the accepted cost at the point it becomes visible: the school that raised the alert cannot close it). All three are handler tier with a non-admin `FakeCurrentUser`; the production stub returns `IsSystemAdmin = true`, so none of this is reachable over HTTP until authentication exists | no | implemented | — |
